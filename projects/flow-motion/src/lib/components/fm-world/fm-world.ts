...
export interface FMItem {
  ...
  addChild: (child: FMItem) => void;
  removeChild: (child: FMItem) => void;
...
@Component({
  selector: 'fm-world',
  imports: [DecimalPipe],
  templateUrl: './fm-world.html',
  styleUrl: './fm-world.scss',
...
export class FMWorld implements FMItem {
  ...
  get inspectableProperties() {
    return FMWorld.inspectableProperties;
  }
  ...
  addChild(child: FMItem) {
    ...
    this.selectionManager.rootChildren.update((c) => [...c, child as FMContainer]);
  }

  removeChild(child: FMItem) {
    this.children.update((c) => c.filter((cChild) => cChild !== child));
    this.selectionManager.rootChildren.update((c) => c.filter((cChild) => cChild !== (child as FMContainer)));
  }
