...
@Directive({
  selector: '[fm-property]',
  standalone: true,
  exportAs: 'fm-property',
  hostDirectives: [
    {
      directive: forwardRef(() => FMContainer),
      inputs: [
        'draggable',
        'fm-container',
        'positioning',
        'fm-container:fm-property',
      ],
    },
  ],
  host: {
...
export class FMProperty {
  ...
  id = input<string>('', { alias: 'fm-property' });
  ...
  constructor() {
    ...
    let currentPreview: Link | null;
    let isFirstMove = true;

    let scale = this.fmService ? this.fmService.scale() : 1;

    drag.dragStart$.pipe(takeUntilDestroyed()).subscribe(() => {
      scale = this.fmService ? this.fmService.scale() : 1;
    ...
    drag.dragMove$.subscribe((move) => {
      if (isFirstMove && this.id()) {
        this.fmService.currentDragSource = this;
        currentPreview = this.fmService.createLink(this.id(), dragPreview);
        isFirstMove = false;
      }
      ...
      let gX = x + parentRect.left - worldRect.left;
      let gY = y + parentRect.top - worldRect.top;
      ...
      this.fmService.updateDragPreview(gX, gY);
    ...
    drag.dragEnd$.subscribe((e) => {
      const targetComp = this.fmService.getComponent(
        e.originalEvent.target
      );

      this.fmService.endDragPreview(this.id(), targetComp?.id());

      if (currentPreview) {
        this.fmService.removePreview(currentPreview);
      }
    });
