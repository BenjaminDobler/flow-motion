...
import { FMContainer, FMProperty, FMWorld } from '../../index';
...
export interface LinkProperties {
  ...
  curveType: WritableSignal<
    'bezier' | 'straight' | 'multi-line' | 'orthogonal' | undefined
  >;
...
type DefaultLinkProperties = Partial<{
  [K in keyof LinkProperties]: LinkProperties[K] extends WritableSignal<infer T>
    ? T
    : never;
}>;

const defaultLinkProperties: DefaultLinkProperties = {
  strokeWidth: 2,
...
export class FMService {
  ...
  defaultProperties = signal<DefaultLinkProperties>(defaultLinkProperties);
  ...
  unregisterDraggableElement(el: FMContainer | FMProperty) {
    this.dragElements.update((x) => x.filter((e) => e !== el));
  }

  getDraggableElementById(id: string) {
  ...
  createLink(
    ...
    const p1 = this.getDraggableElementById(id1);
    ...
    if (typeof id2 === 'string') {
      const p2Property = this.getDraggableElementById(id2);
    ...
    if (p1) {
      ...
      const animate =
        property1?.animatedLink() ||
        linkProperties?.animate ||
        defProps.animate ||
        false;

      const stroke =
        property1?.bondcolor() ||
        linkProperties?.stroke ||
        defProps.stroke ||
        '';
      const strokeWidth =
        property1?.bondstrokewidth() ||
        linkProperties?.strokeWidth ||
        defProps.strokeWidth ||
        2;
      const curveType =
        linkProperties?.curveType || defProps.curveType || 'bezier';
      const curveRadius =
        linkProperties?.curveRadius || defProps.curveRadius || 10;
      ...
      const link: Link = {
        ...
        strokeDasharray: signal(
          linkProperties?.strokeDasharray || defProps.strokeDasharray || '10'
        ),
        ...
        alignTextToPath: signal<boolean>(
          linkProperties?.alignTextToPath || false
        ),
  ...
  getComponent(targetElement: HTMLElement) {
